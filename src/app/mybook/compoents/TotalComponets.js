...
function TotalComponets() {
    ...
    useEffect(() => {
        const totalExpanse = ex.reduce((acc, item) => acc + parseFloat(item.amount), 0);
    ...
    return (
        <div>
            <h6 className='text-center text-pink-500'>Your Balance :- <span className={`ml-2 ${incomeTotle - expanse < 0 ? '!text-red-500' : '!text-green-500'} `}>$ {incomeTotle - expanse}</span></h6>
